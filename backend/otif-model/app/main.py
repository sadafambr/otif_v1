...
# Add otif-genai root to sys.path (sibling of otif-model)
genai_root_path = str(Path(__file__).resolve().parents[2] / "otif-genai")
if genai_root_path not in sys.path:
    sys.path.append(genai_root_path)

genai_summarize_reason = None
_genai_import_error: Optional[str] = None
try:
    from llm.llm_explainer import summarize_reason as genai_summarize_reason
...
def _drivers_from_row(row: Any) -> list[tuple[str, Any, Any]]:
    drivers: list[tuple[str, Any, Any]] = []
    for i in (1, 2, 3):
        if not hasattr(row, "get"):
            continue
        feat = row.get(f"top{i}_feature")
        val = row.get(f"top{i}_value")
        shap_val = row.get(f"top{i}_shap")
        if feat is None or (isinstance(feat, float) and np.isnan(feat)):  # pandas NaN
            continue
        if str(feat).strip() == "":
            continue
        drivers.append((str(feat), val, shap_val))
    return drivers


def _open_explanation_popup(title: str, summary: str, shap_one_liner: str) -> None:
    ...
    if callable(dialog):
        ...
        def _dlg():
            st.markdown("**Summary**")
            st.write(summary)
            st.markdown("**SHAP (one line)**")
            st.write(shap_one_liner)
...
def _run_genai_for_row(*, row: pd.Series, threshold: float, cache_key: str) -> None:
    if genai_summarize_reason is None:
        st.error(f"GenAI module not available. Import error: {_genai_import_error or 'unknown error'}")
        return

    prob_hit_val = row.get("hit_probability", row.get("prob_hit", np.nan))
    try:
        prob_hit = float(prob_hit_val)
    except Exception:
        prob_hit = float("nan")

    prob_miss_val = row.get("prob_miss", row.get("risk_score", np.nan))
    try:
        prob_miss = float(prob_miss_val)
    except Exception:
        prob_miss = float("nan")

    pred_str = row.get("Prediction", row.get("prediction", ""))
    if isinstance(pred_str, str) and pred_str.strip().upper() in ("HIT", "MISS"):
        prediction = 1 if pred_str.strip().upper() == "HIT" else 0
    else:
        prediction = 1 if (np.isfinite(prob_hit) and prob_hit >= threshold) else 0

    if not np.isfinite(prob_hit):
        st.error("Selected row is missing `hit_probability`/`prob_hit`.")
        return
    if not np.isfinite(prob_miss):
    ...
    if cache_key in st.session_state.genai_cache:
        cached = st.session_state.genai_cache[cache_key]
        _open_explanation_popup("🧠 OTIF GenAI Explanation", cached["summary"], cached["shap_one_liner"])
        return

    with st.spinner("Generating GenAI explanation..."):
        try:
            summary, shap_one_liner = genai_summarize_reason(
    ...
    st.session_state.genai_cache[cache_key] = {"summary": summary, "shap_one_liner": shap_one_liner}
    _open_explanation_popup("🧠 OTIF GenAI Explanation", summary, shap_one_liner)
...
def render_shap_tab(config):
    ...
    if pred_path.exists():
        ...
        df_table = df_display.reset_index(drop=True)
        ...
        try:
            ...
            if hasattr(event, "selection") and getattr(event.selection, "rows", None):
                if len(event.selection.rows) > 0:
                    selected_row = df_table.iloc[int(event.selection.rows[0])]
        except TypeError:
            selection_supported = False
            st.dataframe(df_table, use_container_width=True)

        st.info("💡 Select a row to generate a GenAI popup with full summary + one-line SHAP explanation.")

        if selection_supported:
            if selected_row is None:
                st.caption("Select a row in the table above to enable GenAI explanation.")
            else:
                if st.button("🧠 Explain selected row (GenAI)", key="genai_explain_local"):
                    cache_key = f"{selected_month}:{selected_row.get('Sales order', '')}:{selected_row.get('SO Line', '')}:{selected_row.name}"
                    _run_genai_for_row(row=selected_row, threshold=float(artifacts.get("threshold", 0.5)), cache_key=cache_key)
        else:
            if len(df_table) > 0:
                idx = st.number_input(
                    "Row number to explain",
                    min_value=0,
                    max_value=max(0, len(df_table) - 1),
                    value=0,
                    step=1,
                    key="genai_row_idx_local",
                )
                if st.button("🧠 Explain row (GenAI)", key="genai_explain_local_fallback"):
                    row = df_table.iloc[int(idx)]
                    cache_key = f"{selected_month}:{row.get('Sales order', '')}:{row.get('SO Line', '')}:{int(idx)}"
                    _run_genai_for_row(row=row, threshold=float(artifacts.get("threshold", 0.5)), cache_key=cache_key)
            ...
            if st.button("Generate Predictions") or st.session_state.upload_result is not None:
                if st.session_state.upload_result is None:
                    ...
                    with st.status("Processing & Scoring Batch Data...", expanded=True) as status:
                        ...
                        from src.feature_engineering import (add_safe_features, apply_miss_rate_maps, 
                                                            apply_rate_map, add_order_complexity_features, 
                                                            add_tolerance_risk_features, add_interaction_stack_features)
                        ...
                        if 'state_map' in fe_art:
                            df_fe = apply_rate_map(df_fe, "State - Province", "f_state_miss_rate", fe_art.get('g_state', 0.2), fe_art['state_map'])
                        ...
                        if 'imputation' in fe_art:
                            # CRITICAL: Ensure all features expected by the model are present
                        ...
                        output_df['hit_probability'] = probs_hit
                        output_df['risk_score'] = 1.0 - probs_hit
                        output_df['prediction'] = np.where(probs_hit >= artifacts['threshold'], "HIT", "MISS")
                        ...
                        if target_col_raw in df_input.columns:
                            ...
                            y_true_numeric = (df_input[target_col_raw].astype(str).str.strip().str.lower() == "hit").astype(int)
                        ...
                        enhanced_df = pd.concat([output_df.reset_index(drop=True), top_shap_df.reset_index(drop=True)], axis=1)
                ...
                with dist_col2:
                    ...
                    df_head = df_disp.head(100).reset_index(drop=True)
                    selected_upload_row: Optional[pd.Series] = None
                    upload_select_supported = True
                    try:
                        ev = st.dataframe(
                            df_head,
                            use_container_width=True,
                            on_select="rerun",
                            selection_mode="single-row",
                            key="upload_predictions_table",
                        )
                        if hasattr(ev, "selection") and getattr(ev.selection, "rows", None):
                            if len(ev.selection.rows) > 0:
                                selected_upload_row = df_head.iloc[int(ev.selection.rows[0])]
                    except TypeError:
                        upload_select_supported = False
                        st.dataframe(df_head, use_container_width=True)

                    if upload_select_supported and selected_upload_row is not None:
                        if st.button("🧠 Explain selected upload row (GenAI)", key="genai_explain_upload"):
                            cache_key = f"upload:{selected_model_date}:{selected_upload_row.name}"
                            _run_genai_for_row(
                                row=selected_upload_row,
