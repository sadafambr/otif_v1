...
try:
    ...
    from llm.llm_explainer import FEATURE_NAME_MAP as _genai_FEATURE_NAME_MAP
...
if _genai_FEATURE_NAME_MAP is not None:
    FEATURE_NAME_MAP = _genai_FEATURE_NAME_MAP
else:
    FEATURE_NAME_MAP = {
...
def _translate_feature_name(name: str) -> str:
    ...
    if key in FEATURE_NAME_MAP:
        return FEATURE_NAME_MAP[key]["business_name"]
    # Case-insensitive and prefix fallback
    key_lower = key.lower()
...
def _generate_risk_drivers(req: OrderSummaryRequest, prob_miss: float) -> List[RiskDriver]:
    ...
    if shap_features:
        ...
        for rank, (feat, val, shap_val) in enumerate(shap_features, start=1):
            ...
            # Calculate percentage impact (relative to max)
            shap_percentage = round((abs_shap / max_abs_shap) * 100) if max_abs_shap > 0 else 0
            ...
            drivers.append(
                RiskDriver(
                    ...
                    explanation=(
                        f"{human_name} has a {shap_percentage}% impact on miss risk, "
                        f"{'increasing' if (shap_val or 0) > 0 else 'decreasing'} probability."
...
def admin_shap_summary(month: Optional[str] = None) -> dict:
    ...
    if shap_path.exists():
        ...
        for _, row in df.iterrows():
            shap_summary.append(
                {
                    "feature": _translate_feature_name(str(row.get("feature", ""))),
